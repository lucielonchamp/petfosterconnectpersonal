...
export async function getRole(request: Request, response: Response): Promise<any> {
    ...
    try {
        ...
        return response.status(200).json({success: true, message: 'Roles correctly received', data: roles})
    } catch(error) {
...
export async function createRole(request: Request, response: Response): Promise<any> {
    ...
    try{
     ...
     return response.status(201).json({success: true, message: 'Role successfully created', data: newRole})
    } catch(error) {
        return response.status(500).json({
            success: false, message: 'Server error'
        })
